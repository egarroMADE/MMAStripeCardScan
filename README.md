...
This module extends the standalone Stripe CardScan product to scan DNI numbers.
...
# Example
See the `mmastripecardscan-example` directory for an example application that you can try for yourself!

# Integration
* In app/build.gradle, add these dependencies:
    ```gradle
    dependencies {
        implementation 'com.mademediacorp:mmastripecardscan:$stripeVersion'
    }
    ```
...
To do so, configure your app's dependency on mmastripecardscan as follows.
```
    implementation('com.mademediacorp:mmastripecardscan:$stripeVersion') {
      exclude group: 'com.mademediacorp', module: 'ml-core-cardscan' // exclude the cardscan-specific portable tflite runtime
    }
